...
function Story({item, index, onRate, onSave, onShare, saved}) {
  ...
  return <article ref={tileRef} className={`tile tile-${type} tile-pattern-${index % 9} ${item.image ? "tile-has-image" : "tile-no-image tile-text-art"} ${categoryClass(item.section)}`}>
...
export default function Home() {
  ...
  return <main className={`shell daypart-${daypart}`}>
    ...
    {profile && <section className="editionFingerprint"><b>Your edition is drawing from</b><div>{data?.activeSourcePacks?.length ? data.activeSourcePacks.map(pack => <span key={pack.id}>{pack.label}</span>) : <span>your favorite things</span>}<span>Better Start discoveries</span></div></section>}
