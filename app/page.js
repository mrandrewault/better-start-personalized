...
const savedPlaces = () => { try { const value = JSON.parse(localStorage.getItem("upwardsReaderPlaces") || "[]"); return Array.isArray(value) ? value.slice(0, 20).join("|") : ""; } catch { return ""; } };
const prioritizeUnseen = items => {
  const seen = new Set(recentHistory("upwardsReaderStoryHistory").map(entry => entry.id));
...
function playJoyTone(frequency) {
  ...
  if (!Audio) return;
  const context = window.__upwardsAudio || (window.__upwardsAudio = new Audio());
...
export default function Home() {
  ...
  useEffect(() => {
    setSaved(JSON.parse(localStorage.getItem("upwardsReaderSaved") || "[]")); setJoyHistory(recentHistory("upwardsReaderJoyHistory"));
    const priorPalette = Number(localStorage.getItem("upwardsPaletteIndex") || "-1"), nextPalette = (priorPalette + 1) % EDITION_PALETTES.length;
    localStorage.setItem("upwardsPaletteIndex", String(nextPalette)); setPaletteIndex(nextPalette);
    let activeProfile = null;
    try { activeProfile = JSON.parse(localStorage.getItem(PROFILE_KEY) || "null"); } catch {}
    setProfile(activeProfile);
    let lastLoad = Date.now();
    const loadEdition = async preserve => {
      const visit = `${Math.floor(Date.now() / EDITION_MS)}-${Date.now()}-${Math.random()}`, mediaHistory = recentHistory("upwardsReaderMediaHistory"), avoid = [...new Set(mediaHistory.map(entry => entry.id))].slice(-120).join(","), places = savedPlaces(), profileTerms = activeProfile ? [...(activeProfile.broadInterests || []), ...(activeProfile.specificInterests || []), ...(activeProfile.details || []), ...(activeProfile.anythingElse || [])].slice(0, 48).join("|") : "";
      try { const today = new Date().toISOString().slice(0, 10), priorDay = localStorage.getItem("upwardsReaderDay"), hardRefresh = priorDay !== today; const next = await (await fetch(`/api/feed?visit=${encodeURIComponent(visit)}&avoid=${encodeURIComponent(avoid)}&places=${encodeURIComponent(places)}&interests=${encodeURIComponent(profileTerms)}`, {cache: "no-store"})).json(); localStorage.setItem("upwardsReaderDay", today); setJoyHistory(recentHistory("upwardsReaderJoyHistory")); setData(previous => prepareEdition(next, previous, preserve && !hardRefresh)); setEditionNote(`${preserve && !hardRefresh ? "Freshened" : "New"} ${new Date().toLocaleTimeString([], {hour: "numeric", minute: "2-digit"})} edition`); lastLoad = Date.now(); } catch {}
    };
    loadEdition(false);
  ...
  useEffect(() => { if (!wall.length) return; const visible = wall.slice(0, batches * BATCH_SIZE), nowSeen = Date.now(), stories = recentHistory("upwardsReaderStoryHistory"), storyIds = new Set(stories.map(entry => entry.id)); visible.filter(item => item.format !== "joy" && !storyIds.has(itemKey(item))).forEach(item => stories.push({id:itemKey(item),ts:nowSeen})); localStorage.setItem("upwardsReaderStoryHistory", JSON.stringify(stories.slice(-1200))); const media = recentHistory("upwardsReaderMediaHistory"), mediaIds = new Set(media.map(entry => entry.id)); visible.filter(item => item.videoId && !mediaIds.has(item.videoId)).forEach(item => media.push({id: item.videoId, ts: nowSeen})); localStorage.setItem("upwardsReaderMediaHistory", JSON.stringify(media.slice(-300))); const joy = recentHistory("upwardsReaderJoyHistory"), joyIds = new Set(joy.map(entry => entry.signature)); visible.filter(item => item.signature && !joyIds.has(item.signature)).forEach(item => joy.push({signature: item.signature, ts: nowSeen})); localStorage.setItem("upwardsReaderJoyHistory", JSON.stringify(joy.slice(-300))); }, [wall, batches]);
  const rate = (item, action) => { const ratings = JSON.parse(localStorage.getItem("upwardsReaderFeedback") || "[]"); ratings.push({url: item.url, title: item.title, source: item.source, action, ts: Date.now()}); localStorage.setItem("upwardsReaderFeedback", JSON.stringify(ratings.slice(-250))); };
  const toggleSave = item => setSaved(current => { const exists = current.some(savedItem => itemKey(savedItem) === itemKey(item)), next = exists ? current.filter(savedItem => itemKey(savedItem) !== itemKey(item)) : [{...item, savedAt: Date.now()}, ...current]; localStorage.setItem("upwardsReaderSaved", JSON.stringify(next.slice(0, 200))); return next.slice(0, 200); });
  ...
  return <main style={paletteStyle} className={`shell daypart-${daypart} ${identityClass}`} data-editorial-identity={data?.editorialIdentity?.label || "Upwards Reader"}>
    <header className="mast"><div><div className="brand">{profile?.title || "Upwards Reader"}</div><div className="edition">Rage-free news, discovery & good times</div></div><div className="mastTools"><a className="personalizeButton" href="/make-it-yours">{profile ? "Tune my edition" : "Make it yours"}</a>{profile && <button className="genericButton" onClick={clearProfile}>Generic Reader</button>}<button className="savedButton" onClick={() => setShowSaved(value => !value)}>Saved <b>{saved.length}</b></button><button className={`radio ${radio ? "radioOn" : ""}`} onClick={() => setRadio(!radio)} aria-label={`Upwards Radio ${radio ? "on" : "off"}`} title="Upwards Radio placeholder"><span>♪</span><small>{radio ? "ON" : "RADIO"}</small></button></div></header>
